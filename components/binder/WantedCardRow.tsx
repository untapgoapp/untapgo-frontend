...
import { CONDITION_LABELS, FINISH_LABELS, type WantedCard } from "@/lib/binder";
  ...
  return (
    <article className="flex min-w-0 gap-4 rounded-row px-1 py-3 sm:gap-5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex flex-wrap gap-1.5 text-xs"><Badge variant="secondary">Quantity {card.quantity}</Badge>{card.minimum_condition ? <Badge variant="outline">{CONDITION_LABELS[card.minimum_condition]} or better</Badge> : null}{card.preferred_language ? <Badge variant="outline">{card.preferred_language.toUpperCase()}</Badge> : null}{card.preferred_finish ? <Badge variant="outline">{FINISH_LABELS[card.preferred_finish]}</Badge> : null}</div>
