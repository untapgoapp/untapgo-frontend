...
export default function CookiesPage(){
  return <LegalPageShell title="Cookie Policy" introduction="UntapGo uses browser storage and related technologies to keep accounts signed in, remember choices and deliver push notifications when requested. It does not currently use advertising or analytics trackers." toc={toc}>
    <LegalSection id="meaning" title="1. Cookies and similar technologies">
      ...
      <p>The current frontend reads and writes localStorage, uses Supabase-managed first-party authentication cookies and registers a Firebase messaging service worker. The repository contains no application use of sessionStorage or application-created IndexedDB databases. Browsers and verified providers may also manage technical storage as part of authentication, map rendering or push delivery.</p>
    </LegalSection>
    ...
    <LegalSection id="inventory" title="3. Current inventory">
      <div className="legal-table-wrap"><table><thead><tr><th>Name or pattern</th><th>Provider / purpose</th><th>Type and party</th><th>Duration</th><th>Consent</th></tr></thead><tbody>
        <tr><td><code>sb-{"<project-ref>"}-auth-token</code> and related chunked Supabase auth cookie names</td><td>Supabase; persist the authenticated session, PKCE verifier and refresh it after email/password or Google sign-in</td><td>Cookie, first-party context</td><td>Until sign-out, account/session expiry or browser data is cleared; token lifetimes are managed by Supabase</td><td>Strictly necessary; no consent panel</td></tr>
